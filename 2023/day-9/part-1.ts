...
const findNextPrediction = (report: number[]): number => {
    ...
    for (let i = differences.length - 1; i > 0; i--) {
        ...
        const previousDifference = previousDifferences[previousDifferences.length - 1];
        console.log('previous difference', previousDifference);

        prediction += previousDifference;
...
for (const report of reports) {
    const predictedValue = findNextPrediction(report);
    sum += predictedValue;
}
